import { ExcaliburMatchers, ensureImagesLoaded, ExcaliburAsyncMatchers } from "../index";

describe('Custom matcher: toEqualImage', () => {
    beforeAll(() => {
        jasmine.addMatchers(ExcaliburMatchers);
        jasmine.addAsyncMatchers(ExcaliburAsyncMatchers);
    });

    it('should exist', () => {
        expect(ExcaliburMatchers.toEqualImage).toBeDefined();
    });

    it('can match equal dimensions images', (done) => {
        let image1 = new Image();
        let image2 = new Image();
        image1.width = 100;
        image1.height = 200;

        image2.width = 100;
        image2.height = 200;
        ensureImagesLoaded(image1, image2).then(([imageData1, imageData2]) => {
            expect(imageData1).toEqualImage(imageData2, .95);
            done();
        });
    });

    it('[async] can match equal dimensions images', async () => {
        let image1 = new Image();
        let image2 = new Image();
        image1.width = 100;
        image1.height = 200;

        image2.width = 100;
        image2.height = 200;

        await expectAsync(image1).toEqualImage(image2);
    });

    it('can negate match equal dimension images', (done) => {
        let image1 = new Image();
        let image2 = new Image();
        image1.width = 100;
        image1.height = 100;

        image2.width = 100;
        image2.height = 200;
        ensureImagesLoaded(image1, image2).then(([imageData1, imageData2])=> {
            expect(imageData1).not.toEqualImage(imageData2, .95)
            done();
        });

    });

    it('[async] can negate match equal dimension images', async () => {
        let image1 = new Image();
        let image2 = new Image();
        image1.width = 100;
        image1.height = 100;

        image2.width = 100;
        image2.height = 200;
        await expectAsync(image1).not.toEqualImage(image2, .95)
    });

    it('can match pixel by pixel images', (done) => {
        let image1 = new Image();
        let image2 = new Image();
        image1.src = 'data:image/png;base64,';
        image2.src = 'data:image/png;base64,';

        ensureImagesLoaded(image1, image2).then(([imageData1, imageData2])=> {
            expect(imageData1).toEqualImage(imageData2, .95)
            done();
        });
    });

    it('[async] can match pixel by pixel images', async () => {
        let image1 = new Image();
        let image2 = new Image();
        image1.src = 'data:image/png;base64,';
        image2.src = 'data:image/png;base64,';

        await expectAsync(image1).toEqualImage(image2, .95)
    });

    it('can negate match pixel by pixel images', (done) => {
        let image1 = new Image()
        image1.src = 'data:image/png;base64,';
        let image2 = new Image();
        image2.src = 'data:image/png;base64,';

        ensureImagesLoaded(image1, image2).then(([imageData1, imageData2])=> {
            expect(imageData1).not.toEqualImage(imageData2, .99)
            done();
        });
    });

    it('[async] can negate match pixel by pixel images', async () => {
        let image1 = new Image();
        let image2 = new Image();
        image1.src = 'data:image/png;base64,';
        image2.src = 'data:image/png;base64,';

        await expectAsync(image1).not.toEqualImage(image2, .99)
    });

    it('can load images from file', (done) => {
        let image1 = new Image();
        image1.src = 'data:image/png;base64,';
        
        ensureImagesLoaded('./test-images/working.png', image1).then(([imageData1, imageData2]) => {
            expect(imageData1).toEqualImage(imageData2, 1);
            done();
        });
    });

    it('[async] can load images from file', async () => {
        let image1 = new Image();
        image1.src = 'data:image/png;base64,';
        
        await expectAsync('./test-images/working.png').toEqualImage(image1, 1);
    });

    it('can negate images from a file', (done) => {
        ensureImagesLoaded('./test-images/working.png', './test-images/busted.png').then(([imageData1, imageData2])=> {
            expect(imageData1).not.toEqualImage(imageData2, 1);
            done();
        });
    });

    it('[async] can negate images from a file', async () => {
        await expectAsync('./test-images/working.png').not.toEqualImage('./test-images/busted.png', 1);
    });
    
});